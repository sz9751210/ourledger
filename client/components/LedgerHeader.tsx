...
export const LedgerHeader: React.FC = () => {
  ...
  const handleDeleteLedger = (e: React.MouseEvent, ledgerId: string, ledgerName: string) => {
    e.stopPropagation(); // 阻止冒泡，避免觸發「切換帳本」
          ...
          {isMenuOpen && (
            <div className="absolute top-full left-1/2 -translate-x-1/2 md:left-0 md:translate-x-0 mt-2 w-72 bg-white dark:bg-stone-800 rounded-2xl shadow-xl border border-stone-100 dark:border-stone-700 overflow-hidden py-2 animate-in fade-in zoom-in-95 duration-200 z-50 max-h-[70vh] overflow-y-auto no-scrollbar">
              {!isCreating && ledgers.map(ledger => (
                <div key={ledger.id} className="relative group">
                  <button
                    key={ledger.id}
                    onClick={() => {
                  ...
                  {ledgers.length > 1 && ( // 只有大於一本時才顯示刪除鈕
                    <button
                      onClick={(e) => handleDeleteLedger(e, ledger.id, ledger.name)}
                      className="absolute right-10 top-1/2 -translate-y-1/2 p-1.5 text-stone-300 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-full opacity-0 group-hover:opacity-100 transition-all"
                      title={t('delete')}
                    >
                      <Trash2 size={14} />
                    </button>
