...
import { Search, Filter, X, Hash, Pin } from 'lucide-react';
...
export const TransactionList: React.FC<TransactionListProps> = ({ onEdit }) => {
  const { activeLedgerId, getLedgerExpenses, currentUser, getMember, baseCurrency, convertAmount, categories, togglePinExpense, t } = useAppStore();
          ...
          return (
            ...
            >
              <div className="flex items-center space-x-4">
                <div className={`relative w-12 h-12 rounded-2xl flex items-center justify-center transition-transform group-hover:scale-110 ${category?.color || 'bg-gray-100'}`}>
                  <Icon size={20} />
                  {expense.isPinned && (
                  ...
                  {expense.tags && expense.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-1.5">
                      {expense.tags.slice(0, 3).map((tag, idx) => (
                        <span key={idx} className="inline-flex items-center gap-0.5 px-1.5 py-0.5 bg-stone-100 dark:bg-stone-800 text-stone-500 dark:text-stone-400 text-[9px] font-bold rounded-md">
                          <Hash size={8} />{tag}
                        </span>
                      ))}
                      {expense.tags.length > 3 && (
                        <span className="text-[9px] text-stone-400">+{expense.tags.length - 3}</span>
                      )}
                    </div>
                  )}
                </div>
