...
import { X, Trash2, Sparkles, Loader2, Plus, ChevronLeft, Check, AlignLeft, Image as ImageIcon, RefreshCcw, Search, ScanLine, Calendar as CalendarIcon, Tag, Wallet, Users } from 'lucide-react';
...
export const AddExpense: React.FC<AddExpenseProps> = ({ onClose, initialExpense }) => {
  ...
  const handleCreateCategory = () => {
    if (newCatName.trim()) {
      const id = addCategory({ name: newCatName, icon: newCatIcon, color: newCatColor });
  ...
  const SplitOption = ({ type, label, active }: { type: SplitType, label: string, active: boolean }) => (
    <button
      ...
      className={`py-3 px-2 text-xs font-bold rounded-xl border transition-all whitespace-nowrap flex-1 ${
        active 
                     ...
                     <div className="grid grid-cols-2 gap-2">
                       {activeLedgerMembers.map(member => (
                          <button
                            ...
                            className={`flex items-center justify-center gap-2 py-3 px-3 rounded-xl text-sm font-bold transition-all ${
                              paidBy === member.id 
                      ...
                      {displayedIcons.map(iconName => {
                        ...
                        return (
                          <button
                            ...
                            title={iconName}
                            className={`p-2 rounded-xl flex items-center justify-center transition-all aspect-square ${
                              newCatIcon === iconName
                    ...
                    <div className="grid grid-cols-6 gap-2">
                      {AVAILABLE_COLORS.map(colorClass => (
                        <button
                          ...
                          className={`h-10 rounded-xl transition-all relative ${colorClass} ${
                            newCatColor === colorClass ? 'ring-2 ring-stone-800 dark:ring-white ring-offset-2 dark:ring-offset-stone-900' : 'opacity-70 hover:opacity-100'
                          }`}
                        >
